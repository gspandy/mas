...
import com.letv.mas.caller.iptv.tvproxy.common.util.PagePeriodControl;
 ...
 */
@XmlRootElement
public class PagePeriodResponse<T> extends BaseResponse {
    ...
    private T items; // 当前页的数据

    public PagePeriodResponse() {
    }

    public PagePeriodResponse(PagePeriodControl<T> pc) {
        if (pc != null) {
    ...
    public PagePeriodResponse(PageControlInfo<T> pc) {
        if (pc != null) {
            this.setCount(pc.getCount());
            this.setCurrentPage(pc.getCurrentPage());
