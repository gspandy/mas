package com.letv.mas.caller.iptv.tvproxy.common.model.dao.tp.report;
...
import com.letv.mas.caller.iptv.tvproxy.common.util.CommonUtil;
...
public class ActReportDao extends BaseTpDao {
     ...
     * 接口功能：防盗链行为上报接口
     */
    private final String URI_ANTI_REPORT = ApplicationUtils.get(ApplicationConstants.ACT_REPORT_ANTI_SET_URI);

    /**
     * 接口功能：防盗链行为上报更新接口
     */
    private final String URI_ANTI_OPT = ApplicationUtils.get(ApplicationConstants.ACT_REPORT_ANTI_OPT_URI);
    ...
    public ReportTpResponse setAntiReport(String splatid, CommonParam commonParam) {
        ...
        StringBuilder url = new StringBuilder(this.URI_ANTI_REPORT);
        try {
            ...
            // log.info("[ActReportDao.setAntiReport]:url=" + url + ", result="
            // + response.getCode());
            actReportLog.info("|[setAntiReport]|" + url + "|" + result);
        } catch (Exception e) {
            response.setCode(-1);
            log.error(
                    "[ActReportDao.setAntiReport]:url=" + url + ",result=" + response.getCode() + ",error="
                            + e.getMessage(), e);
    ...
    public ReportTpResponse updateAntiReport(int type, String key, String value, int ttl) {
        ReportTpResponse response = new ReportTpResponse();
        StringBuilder url = new StringBuilder(this.URI_ANTI_OPT);
        try {
            ...
            // log.info("[ActReportDao.updateAntiReport]:url=" + url +
            // ", result=" + response.getCode());
            actReportLog.info("|[setAntiReport]|" + url + "|" + result);
        } catch (Exception e) {
            response.setCode(-1);
            log.error(
                    "[ActReportDao.updateAntiReport]:url=" + url + ",result=" + response.getCode() + ",error="
                            + e.getMessage(), e);
            actReportLog.info("|[setAntiReport]|" + url + "|" + -1);
        }
        return response;
