...
package com.letv.mas.manager.config;
 ...
 */
@ConfigurationProperties(DEurekaClientConfigBean.PREFIX)
public class DEurekaClientConfigBean implements EurekaClientConfig {

    private static final Logger logger = LoggerFactory.getLogger(DEurekaClientConfigBean.class);
    ...
    private AtomicReference<Map> updateEurekaServer = new AtomicReference<Map>();

    public void updateEurekaServer(Map<String, String> updateMap) {
        if (updateMap != null) {
            updateEurekaServer.set(updateMap);
        }
    }

    @Override
    public String[] getAvailabilityZones(String region) {
        Map<String, String> updateMap = updateEurekaServer.get();
        if (updateMap != null && !updateMap.isEmpty()) {
            String zones = myZone;
            for (Map.Entry<String, String> entry : updateMap.entrySet()) {
                if (!myZone.equals(entry.getKey())) {
                    zones = zones + "," + entry.getKey();
                }
            }
            return zones.split(",");
        }
    ...
    private void initFormOutSide() {
		/*Map result = this.restTemplate.getForObject("", Map.class);
		Map<String, String> aZones = (Map<String, String>) result.get(region);
		String zones = "";
		for (Map.Entry<String, String> entry : aZones.entrySet()) {
			zones = zones + entry.getKey();
			this.serviceUrl.put(entry.getKey(), entry.getValue());
		}
		availabilityZones.put(region, zones);*/
    }
    ...
    public List<String> getEurekaServerServiceUrls(String myZone) {
        ...
        if (updateMap != null && !updateMap.isEmpty()) {
            serviceUrls = updateMap.get(myZone);
        } else {
            serviceUrls = this.serviceUrl.get(myZone);
        ...
        if (!StringUtils.isEmpty(serviceUrls)) {
            ...
            for (String eurekaServiceUrl : serviceUrlsSplit) {
                ...
                if (local != null) {
                    if (eurekaServiceUrl.contains(local)) {
                        continue;
                    }
                }
    ...
    public boolean equals(Object o) {
        ...
        DEurekaClientConfigBean that = (DEurekaClientConfigBean) o;
    ...
    public void timeUpdateEurekaServer() {
        EurekaClientConfig config = DiscoveryManager.getInstance().getEurekaClientConfig();
        if (config instanceof DEurekaClientConfigBean && serverName != null && !serverName.equals("")) {
            ...
            updateEurekaServer(map);
            logger.info("updateEurekaServer......");
        }
    }
}
